...
node1()->
    ...
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"erl -sname 10250 -setcookie abc -detached ",10000)}]),
    timer:sleep(5000),
...
%    io:format("~p~n",[{?MODULE,?LINE,ssh_sftp:del_dir(ChannelPid,VmId)}]),
 %   io:format("~p~n",[{?MODULE,?LINE,ssh_sftp:del_dir(ChannelPid,"erl_crasch.dump")}]),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"rm -rf "++VmId++" erl_crasch.dump include "++ServiceId,10000)}]),
    timer:sleep(2000),
    ?debugMsg("rm -rf "++VmId++" erl_crasch.dump"),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"mkdir "++VmId,10000)}]),
 ...
 %   ?assertMatch(ok,ssh_sftp:make_dir(ChannelPid,VmId)),
  ...
  %  io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"git clone https://joq62:20Qazxsw20@github.com/joq62/include.git "++VmId,10000)}]),
    ...
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"git clone https://joq62:20Qazxsw20@github.com/joq62/"++ServiceId++".git",10000)}]),
    timer:sleep(1000),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"mv "++ServiceId++" "++VmId,10000)}]),
  %  timer:sleep(200),
 ...
 %  io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"git clone https://joq62:20Qazxsw20@github.com/joq62/include.git",10000)}]),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"git clone https://github.com/joq62/include.git",10000)}]),
   timer:sleep(2000),
  %  io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"mv include "++VmId,10000)}]),
    timer:sleep(200),
  ...
  %  ?debugMsg("ls "),
   % timer:sleep(200),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"cp "++VmId++"/"++ServiceId++"/src/"++ServiceId++".app "++VmId++"/"++ServiceId++"/ebin",10000)}]),
    timer:sleep(200),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd, "erlc -o "++VmId++"/"++ServiceId++"/ebin "++VmId++"/"++ServiceId++"/src/*.erl",10000)}]),
    timer:sleep(200),
    
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"erl -pa "++VmId++"/"++ServiceId++"/ebin "++"-sname "++VmId++" -setcookie abc -detached ",10000)}]),
    timer:sleep(5000),
