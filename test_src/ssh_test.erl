...
%    io:format("~p~n",[{?MODULE,?LINE,ssh_sftp:del_dir(ChannelPid,VmId)}]),
 ...
 %   io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"mv "++ServiceId++" "++VmId,10000)}]),
  ...
  %  ?debugMsg("ls "),
   % timer:sleep(200),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"cp "++VmId++"/src/"++ServiceId++".app "++VmId++"/ebin",10000)}]),
    timer:sleep(200),
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd, "erlc -I "++VmId++"/include -o "++VmId++"/ebin "++VmId++"/src/*.erl",10000)}]),
    timer:sleep(200),
    
    io:format("~p~n",[{?MODULE,?LINE,my_ssh:ssh_send(Ip,Port,User,PassWd,"erl -pa "++VmId++"/ebin "++"-sname "++VmId++" -setcookie abc -detached ",10000)}]),
    timer:sleep(5000),
