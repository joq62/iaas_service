...
node1()->
    ...
    % Ok
    {ok,ConRef,ChanId}=my_ssh:connect(Ip,Port,User,PassWd),
    ?assertMatch(["/home/pi"],my_ssh:send(ConRef,ChanId,"pwd")), 
    ?assertMatch(ok,my_ssh:close(ConRef)),
   
     % Error 
    ?assertMatch({error,ehostunreach},my_ssh:connect("192.168.66.66",Port,User,PassWd)), 
    ?assertMatch({error,econnrefused},my_ssh:connect(Ip,1234,User,PassWd)),
    ?assertMatch({error,"Unable to connect using the available authentication methods"},my_ssh:connect(Ip,Port,"glurk",PassWd)),  
    ?assertMatch({error,"Unable to connect using the available authentication methods"},my_ssh:connect(Ip,Port,User,"glurk")),  
    
    % ok
    {ok,ConRef1,ChanId1}=my_ssh:connect(Ip,Port,User,PassWd),
    ?assertMatch(["/home/pi"],my_ssh:send(ConRef1,ChanId1,"pwd")),  
